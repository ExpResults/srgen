...
void DependencyForest::reset(int N) {
  DependencyTree::reset(N);
...
void DependencyForest::rootize(int now, int root_flag, int depth) {
  root_[now] = root_flag;
  depth_[now] = depth;
...
int DependencyForest::root(int i) const {
  return root_[i];
}


int DependencyForest::depth(int i) const {
  return depth_[i];
}


int DependencyForest::lca(int u, int v) const {
  if (u > v) {
    std::swap(u, v);
  }
  return lca_[u][v];
...
bool DependencyForest::is_root(int i) const {
  return (root_[i] == i);
}


bool DependencyForest::same_ancestor(int u, int v) const {
